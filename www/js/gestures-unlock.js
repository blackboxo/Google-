...
var GesturesUnlock = function () {
  ...
  this.arcDidColor = 'rgba(255, 255, 255, .8)'; // 手指触摸圆的颜色
  this.arcLineWidth = 6; // 圆的粗细
  this.touchColor = 'rgba(255, 255, 255, .4)'; // 手指触摸的线条颜色
  this.touchLineWidth = 15; // 手指触摸的线条粗细
  this.pwdResult = ''; // 密码结果
...
GesturesUnlock.prototype.pointArc = function (ctx, x, y, radius, color, lineWidth) {
  this.drawArc(ctx, x, y, radius, 'rgba(255, 255, 255, .2)');
  this.drawArc(ctx, x, y, radius, color, lineWidth);
  this.drawArc(ctx, x, y, radius / 5, color);
...
GesturesUnlock.prototype.ninePointArc = function (ctx) {
  ...
  for (key in pointArr) {
    ...
    this.drawArc(ctx, pointArr[key].x, pointArr[key].y, this.radius / 5, this.arcColor);
  }
}
  ...
  canvas.addEventListener('touchmove', function (eve) {
    ...
    for (key in pointArr) {
      ...
      // 如果不是点位圆的坐标,则线段跟着手指移动
        ...
        thisObj.drawLine(ctx, thisObj._tempPoint.x, thisObj._tempPoint.y, touchX, touchY, 'rgba(255, 255, 255, .09)', thisObj.touchLineWidth);
      }
    }
